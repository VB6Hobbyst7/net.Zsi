...
'Carica il filtro di stampa 'NomeFiltro' leggendolo dal corrispondente file .SS2
Sub PrintBlaster(NomeFiltro As String, HelpContextID As Long, Optional DSNDitta As Variant, Optional bolFiltroPerTemp As Boolean = False)
    Dim FRM As New FrmFiltro
    On Local Error Resume Next
    FRM.strNomeFiltro = NomeFiltro
    FRM.DSNDitta = DSNDitta
    FRM.FiltroConTemp = bolFiltroPerTemp   '<<< Gestione di un Filtro che crea un temporaneo tramite Stored Procedure; al Procedi non verrà passato il filtro alla stampa
    'Set FRM.Opzioni = MXCR
    FRM.FormProp.FormID = HelpContextID
    On Local Error GoTo out_of_memory
    Load FRM

'per programma chiavi privo di agenti
#If ISCHIAVI = 0 Then
