...
IF OBJECT_ID ('dbo.ZSI_DISPONIBILITAM05') IS NOT NULL
	DROP TABLE dbo.ZSI_DISPONIBILITAM05
GO

CREATE TABLE dbo.ZSI_DISPONIBILITAM05
	(
	CODCLIFOR        VARCHAR (7),
	CODART           VARCHAR (50),
	CODIMBALLO		VARCHAR(10),
	QTAGESTRES       DECIMAL (16, 6) NOT NULL,
	DISPONIBILITA       DECIMAL (16, 6) NOT NULL,
	UTENTEMODIFICA   VARCHAR (25),
	DATAMODIFICA     DATETIME,
	CONSTRAINT PK_ZSI_DISPONIBILITAM05 PRIMARY KEY (CODCLIFOR, CODART)
	)
GO

GRANT DELETE ON dbo.ZSI_DISPONIBILITAM05 TO Metodo98
GO
GRANT INSERT ON dbo.ZSI_DISPONIBILITAM05 TO Metodo98
GO
GRANT REFERENCES ON dbo.ZSI_DISPONIBILITAM05 TO Metodo98
GO
GRANT SELECT ON dbo.ZSI_DISPONIBILITAM05 TO Metodo98
GO
GRANT UPDATE ON dbo.ZSI_DISPONIBILITAM05 TO Metodo98
GO
...
BEGIN
	...
	INSERT INTO dbo.ZSI_DISPONIBILITAM05 (CODCLIFOR, CODART, QTAGESTRES, DISPONIBILITA, CODIMBALLO, UTENTEMODIFICA, DATAMODIFICA)
	SELECT @codclifor, @codart, 0, @disponibilita, @codimballo, 'trm', getdate()
	WHERE NOT EXISTS(SELECT 1 FROM ZSI_DISPONIBILITAM05 x WHERE x.CODCLIFOR = @codclifor AND x.CODART = @codart)
	...
	UPDATE ZSI_DISPONIBILITAM05
	SET QTAGESTRES = (SELECT sum(x.qtagestres) FROM EXCEL_PORTAFOGLIOORDINI x WHERE x.CODCLIFOR = @codclifor AND x.ARTICOLO = @codart AND CODIMBALLO = @codimballo)
...
IF OBJECT_ID ('dbo.ZSI_VISTA_DISPONIBILITAM05') IS NOT NULL
	DROP VIEW dbo.ZSI_VISTA_DISPONIBILITAM05
GO

create view ZSI_VISTA_DISPONIBILITAM05 as 
SELECT A.DSCCONTO1 AS RAGIONESOCIALE
, Z.CODART AS ARTICOLO
, AA.DESCRIZIONE AS DESCRIZIONE
, T.DESCRIZIONE AS IMBALLO
, T.QTA_COLLI AS NRPEZZIIMBALLO
, Z.QTAGESTRES
, CAST(isnull((CASE WHEN CAST(T.QTA_COLLI AS INT) > 0 THEN Z.DISPONIBILITA*CAST(T.QTA_COLLI AS INT) ELSE Z.DISPONIBILITA END), 0) AS INT) AS DISPONIBILITA
, CAST(isnull((CASE WHEN CAST(T.QTA_COLLI AS INT) > 0 THEN Z.QTAGESTRES/CAST(T.QTA_COLLI AS INT) ELSE 0 END), 0) AS INT) AS COLLIDASPEDIRE
, CAST(isnull(Z.DISPONIBILITA, 0) AS INT) AS COLLISPEDIBILI
--, z.* 
FROM ZSI_DISPONIBILITAM05 Z
JOIN ANAGRAFICACF A ON Z.CODCLIFOR = A.CODCONTO
JOIN ANAGRAFICAARTICOLI AA ON AA.CODICE = Z.CODART
LEFT OUTER JOIN TABIMBALLI T ON T.CODICE = Z.CODIMBALLO
GO

GRANT SELECT ON dbo.ZSI_VISTA_DISPONIBILITAM05 TO Metodo98
GO
...
BEGIN
	...
	INSERT INTO dbo.ZSI_CONFEZSPEDIBILE (PROGRESSIVO, IDRIGA, POSIZIONAMENTO, CONFEZIONATO, QTACONFEZIONE, LOTTO, DISPONIBILITA, UTENTEMODIFICA, DATAMODIFICA)
	VALUES (@idtesta, @idriga, '', 0, @qtaspedibile, @nrlotto, 0, 'TRM', GETDATE())
...
FROM 
	TESTEDOCUMENTI T WITH (NOLOCK) JOIN EXTRATESTEDOC ET WITH (NOLOCK) ON T.PROGRESSIVO = ET.IDTESTA 
