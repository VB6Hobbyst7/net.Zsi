        ...
        public static bool SaveColumnsSettings(RadGridView rgv, string s, string name)
        {
            ...
            if (File.Exists(sf))
            {
                try
                {
                    ...
                    {
                        string df = Path.Combine(Path.Combine(Properties.Settings.Default.pathServerApp, "GridLayout"), System.Environment.UserName + string.Format("_{0}.xml", name));
        ...
        public static void check_columns(string s, string name)
        {
            try
            {
                ...
                try
                {
                    if (Directory.Exists(Path.Combine(Properties.Settings.Default.pathServerApp, "GridLayout")))
                    {
                        string sf = Path.Combine(Path.Combine(Properties.Settings.Default.pathServerApp, "GridLayout"), System.Environment.UserName + string.Format("_{0}.xml", name));
