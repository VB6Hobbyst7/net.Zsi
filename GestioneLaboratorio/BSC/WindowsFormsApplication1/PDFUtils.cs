     ...
     public static class PdfImageExtractor
        {
                ...
                using (var reader = new PdfReader(filename))
                {
                    ...
                    for (var i = 1; i <= reader.NumberOfPages; i++)
                    {
                        ...
                        if (listener.Images.Count > 0)
                        {
                            ...
                            {
                                //MessageBox.Show(string.Format("{0}_Page_{1}_Image_{2}{3}",
                                //    Path.GetFileNameWithoutExtension(filename), i.ToString("D4"), index.ToString("D4"), pair.Value));
                                ...
                                string path = Path.Combine(@"C:\Users\alfredo\Google Drive\Lavoro\SignFileRTFPDF", string.Format("{0}_Page_{1}_Image_{2}{3}",
                                    Path.GetFileNameWithoutExtension(filename), i.ToString("D4"), index.ToString("D4"), pair.Value));
                                ((System.Drawing.Image)pair.Key).Save(path); 
            ...
            /// <summary>Extracts all images (of types that iTextSharp knows how to decode) from a PDF file.</summary>
            public static Dictionary<string, System.Drawing.Image> ReplaceImages(string filename, System.Drawing.Image imagefind, System.Drawing.Image imagenew)
            {
                var images = new Dictionary<string, System.Drawing.Image>();
                using (var reader = new PdfReader(filename))
                {
                    ...
                    for (var i = 1; i <= reader.NumberOfPages; i++)
                    {
                        ...
                        if (listener.Images.Count > 0)
                        {
                            ...
                            foreach (var pair in listener.Images)
                            {
                                ...
                                //string path = Path.Combine(System.Windows.Forms.Application.StartupPath, string.Format("{0}_Page_{1}_Image_{2}{3}",
                                //    Path.GetFileNameWithoutExtension(filename), i.ToString("D4"), index.ToString("D4"), pair.Value));
                                //((System.Drawing.Image)pair.Key).Save(path); 
